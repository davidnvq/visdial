# python ~/Dropbox/repos/visdial_v5/data/extract_image_features_detectron.py
import argparse
import glob
import os
...
parser.add_argument(
        ...
        default=[
			# os.path.join(HOME_PATH, "datasets/visdial/VisualDialog_val2018"),
			os.path.join(HOME_PATH, "datasets/visdial/val2014"),
			os.path.join(HOME_PATH, "datasets/visdial/train2014")
			]
		)
parser.add_argument(
		"--save-path",
		help="Path to output file for saving bottom-up features (.h5)",
		default=os.path.join(HOME_PATH, "datasets/visdial/train_mask_rcnn_X-101-32x8d-FPN_1x.h5"),
		)
...
parser.add_argument(
		...
		default=os.path.join(HOME_PATH,
							 "tmp/detectron/configs/12_2017_baselines/e2e_faster_rcnn_X-101-32x8d-FPN_1x.yaml"),
		)
parser.add_argument(
		"--weights",
		help="Path to model weights file saved by Detectron (.pkl)",
		default=os.path.join(HOME_PATH, "tmp/pretrained_detectron/e2e_mask_rcnn_X-101-32x8d-FPN_1x.pkl"),
		)
parser.add_argument(
		"--max-boxes",
...
def main(args):
	...
	image_paths = []
	for root_path in args.image_root:
		image_paths = image_paths + glob(root_path + "/*.jpg")
