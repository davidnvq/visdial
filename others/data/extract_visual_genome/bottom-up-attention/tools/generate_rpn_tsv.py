...
# Example:
# ./tools/generate_tsv.py --gpu 0 --cfg experiments/cfgs/faster_rcnn_end2end_resnet.yml --def models/vg/ResNet-101/faster_rcnn_end2end_final/test_rpn.prototxt --out test2014_resnet101_faster_rcnn_genome.tsv --net data/resnet101_faster_rcnn_final_iter_320000.caffemodel --split /home/quanguet/datasets/visdial/VisualDialog_val2018
# ./tools/generate_rpn_tsv.py --gpu 0 --cfg experiments/cfgs/faster_rcnn_end2end_resnet.yml --def models/vg/ResNet-101/faster_rcnn_end2end_final/test_rpn.prototxt --out test2014_resnet101_faster_rcnn_genome.tsv --net data/resnet101_faster_rcnn_final_iter_320000.caffemodel --split /home/quanguet/datasets/visdial/VisualDialog_val2018

import _init_paths
from fast_rcnn.config import cfg, cfg_from_file
from fast_rcnn.test import im_detect, _get_blobs, im_detect_cls_score
...
def load_image_ids(split_name):
    ...
    if split_name == 'coco_test2014':
        ...
        img_paths = glob(os.path.join(split_name, "*.jpg"))
        print("img_paths", img_paths)
        img_ids = [image_id_from_path(img_path) for img_path in img_paths]
        for img_path, img_id in zip(img_paths, img_ids):
            split.append((img_path, img_id))

    return split

def get_detections_from_im_chop(net, im_file, image_id, conf_thresh=0.2):
    im = cv2.imread(im_file)
    scores = im_detect_cls_score(net, im)
        ...
        with open(outfile, 'ab') as tsvfile:
            ...
            for im_file, image_id in image_ids:
                if int(image_id) in missing:
                    _t['misc'].tic()
                    writer.writerow(get_detections_from_im_chop(net, im_file, image_id))
                    _t['misc'].toc()
                    if (count % 10) == 0:
                        print 'GPU {:d}: {:d}/{:d} {:.3f}s (projected finish: {:.2f} hours)'.format(gpu_id, count + 1, len(missing), _t['misc'].average_time,
