#!/usr/bin/env bash
# To run:
# bash /home/ubuntu/Dropbox/repos/visdial/scripts/dais_train_lf_gen.sh

OVERFIT=true
ROOT=/home/ubuntu
DATASET=$ROOT/datasets/visdial
CKPOINT=$ROOT/checkpoints/lf_gen

PATH_LOAD=$CKPOINT/lf_gen_faster_rcnn_x101_trainval.pth
PATH_PROJ=$ROOT/Dropbox/repos/visdial
CONFIG=$PATH_PROJ/configs/lf_gen_faster_rcnn_x101.yml

if [ $OVERFIT = true ]; then
    PATH_SAVE=$CKPOINT/tmp
    COMET=test
else
    PATH_SAVE=$CKPOINT/lf_gen_`date +%d%b%Y`
    COMET=visdial_lf_gen
fi
