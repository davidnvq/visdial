#!/usr/bin/env bash
# To run:
# bash /home/ubuntu/Dropbox/repos/visdial/scripts/dais_train_attn_gen.sh

OVERFIT=true
ROOT=/home/ubuntu
DATASET=$ROOT/datasets/visdial
CKPOINT=$ROOT/checkpoints/attn_gen

PATH_LOAD=$CKPOINT/attn_gen_faster_rcnn_x101_trainval.pth
...
if [ $OVERFIT = true ]; then
    ...
    PATH_SAVE=$CKPOINT/attn_gen_`date +%d%b%Y`
    COMET=visdial_attn_gen
fi
