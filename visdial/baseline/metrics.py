...
class Monitor(object):
	...
	def get_elem_dict(self, elem):
		...
		gtr_scores, gtr_idx = torch.sort(elem['gt_relevance'], descending=True)
		scores = [[] for _ in range(10)]
		attn_weights = [[] for _ in range(10)]

		gtr_scores = gtr_scores[:10]
		gtr_idx = gtr_idx[:10]

		gtr_texts = [' '.join(dialog[elem['round_id'] - 1]['answer_options'][idx])
		             for idx in gtr_idx]
	...
	def update(self, batch_img_ids, batch_logits, batch_targets, attn_weights):
