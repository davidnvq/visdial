...
class Encoder(nn.Module):
    ...
    def __init__(self, text_encoder, img_encoder, attn_encoder, hidden_size):
        ...
        nn.init.kaiming_uniform_(self.encoder_linear[0].weight)
        nn.init.kaiming_uniform_(self.encoder_linear[2].weight)
        nn.init.constant_(self.encoder_linear[0].bias, 0)
        nn.init.constant_(self.encoder_linear[2].bias, 0)
    ...
    def forward(self, batch):
        ...
        # [BS x NH, HS] hist
        # hist = self.summaries[1](hist, hist_mask)

        # [BS x NH, HS] ques
        ques = self.summaries[0](ques, ques_mask)

        # [BS x NH, HS x 2]
        # encoder_out = torch.cat([img, hist, ques], dim=-1)
        # In version 4, we concat [img, ques] only, there is no need for hist
